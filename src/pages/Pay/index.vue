...
export default {
  ...
  methods:{
    ...
    // 1、点击按钮能够弹出消息盒子(配置盒子)
    // 2、生成二维码图片（请求回来的支付信息当中包含了一个生成二维码所有的数据（微信给我们的一个链接））
    // 3、当弹出二维码的时候，那么必须开始去检查用户对这个订单的支付状态
      // 当支付状态为已支付，那么就自动跳转到支付成功界面
      // 当支付状态为未支付，那么就停在原地，就算点击成功支付，也不能跳转，但是取消会把支付检查取消掉
        ...
        this.$alert(`<img src="${imgUrl}" />`, '请使用微信扫码支付', {
          ...
          beforeClose:(action, instance, done)=>{
            ...
            if(action === 'confirm'){
              ...
              //   // warning 代表警告 橙色
              //   // info 代表提示 灰色
              //   this.$message.info('请确保支付成功，支付成功会自动跳转')
              // }
          ...
          .then() // 点击了确认按钮后的操作
          .catch(); // 点击了取消按钮后的操作
          // 无论是点击确认按钮还是点击取消按钮，都会强制的关闭mesageBox
          ...
          this.timer = setInterval(async () => {
            ...
            if(result.code === 200){
              /* 4、如果支付成功 */
                ...
                // 2）清除定时器
                // 3）关闭消息盒子
                // 4）存储成功状态 是为了后期用户点击按钮的时候判断依据

              // 把支付状态码存到data当中 用于用户点击按钮的时候判断是否支付完成
              this.payStatus = 200
