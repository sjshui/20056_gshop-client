  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="Trade in getTrade1"
        :key="Trade.id"
      >
        <span class="username" :class="{selected:Trade.isDefault === '1'}">{{ Trade.consignee }}</span>
        <p @click="changeDedault(Trade,getTrade1)">
          <span class="s1">{{ Trade.userAddress }}</span>
          <span class="s2">{{ Trade.phoneNum }}</span>
          <span class="s3" v-if="Trade.isDefault === '1'">默认地址</span>
        </p>
      </div>
      <div class="line"></div>
      ...
      <div class="detail">
        ...
        >
          <li>
            <img :src="goods.imgUrl" alt="" style="width: 80px; height: 80px" />
          </li>
          <li>
            <p>{{ goods.skuName }}</p>
            <h4>7天无理由退货</h4>
...
export default {
  ...
  methods: {
    ...
    // 修改默认收获地址，排它
    changeDedault(Trade,getTrade1){
      getTrade1.forEach(item => item.isDefault = '0')
      Trade.isDefault = '1'
    },
    // 提交订单
    async submitOrder(){
      ...
      let tradeData = {
        consignee: this.getTrade1.consignee,
        consigneeTel: this.getTrade1.phoneNum,
        deliveryAddress: this.getTrade1.userAddress,
        paymentWay: "ONLINE",
      ...
      try {
        ...
        if(result.code === 200){
          // this.orderNo= result.data // 这里是存储订单编号，存不存都行，因为订单编号是要带到下个页面的
  ...
  computed: {
    ...
    ...mapState({
      getTrade1: state => state.trade.trade || {},
      tradeInfo: state => state.trade.tradeInfo || {}
    }),
    getTrade(){
      return this.getTrade1.trade || []
    },
    // 计算最终确定的用户信息,根据上面点击修改默认发生变化的
    defaultUserAddress(){
      let getTrade = this.getTrade1
      return getTrade.find(item => item.isDefault === '1') || {}
    }
  },
}
</script>
