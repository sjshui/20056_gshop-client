  ...
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body">
        <ul
          class="cart-list"
          v-for="shopCart in shopCartLists1"
          :key="shopCart.id"
        >
    ...
    <div class="cart-tool">
      ...
      <div class="option">
        <a href="javascript:;" @click="deleteOne">删除选中的商品</a>
...
export default {
  ...
  methods: {
    ...
    async updateOne(shopCart){
      try {
        ...
        {skuId:shopCart.skuId,isChecked:shopCart.isChecked?0:1})
  ...
  computed: {
    // mapState使用数组必须名字相同，数据只能总的state当中的数据才能使用，模块化后不能使用
    ...mapGetters(["shopCartList"]),
    shopCartLists1() {
      return this.shopCartList.cartInfoList || [];
    },
    checkedNum() {
      return this.shopCartLists1.reduce((prev, item) => {
        if (item.isChecked) {
          prev += item.skuNum;
        }
        return prev;
    ...
    isAllCheck: {
      get() {
        return this.shopCartLists1.every((item) => item.isChecked);
      },
      async set(val) {
        try {
