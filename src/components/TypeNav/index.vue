...
<script>
/* 
    1.dispathch()触发发请求的异步action调用 ==> 数据从后台接口请求到vuex的state中
    2.store.state / mapState()读取vuex的state数据 ==> 数据冲state到组件的computed
    3.在模板中动态显示
  */
...
export default {
  ...
  methods: {
    ...
    /* 显示指定下标的子分类列表 */
    // showSubList:_.throttle((index) ==> {// 不可以，原因在于箭头函数没有自己的this，且不能通过bind来指定自己的this
    showSubList: throttle(
      function (index) {
        // 这个事件监听回调函数调用的频率太高
        // console.log('throttle',index)
        // 只有当还没有离开整个分类的div时才不更新
